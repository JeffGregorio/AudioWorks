...
@synthesize tdScopeView;
@synthesize fdScopeView;
@synthesize tdControlArray;
@synthesize fdControlArray;
...
- (void)setUpDistortionControl {
    ...
    [control setHorizontalRange:plotXPos max:plotXPos];
    ...
    idx = [tdControlArray addControlWithStyle:kMETControlStyleRect
                                       values:CGPointMake(plotXPos, kInitialClippingAmplitudeLow)];
    control = [tdControlArray getControlAtIndex:idx];
    [control setConstrainVerticallyToParentView:true];
    [control setConstrainHorizontallyToParentView:true];
...
- (void)setUpHelp {
    ...
    bbl = [[HelpBubble alloc] initWithText:@"When using effects, the blue signal is the input (dry) and the red signal is the output (wet)"
                                    origin:origin
                                     width:300.0f];
    ...
    bbl = [[HelpBubble alloc] initWithText:@"Move the control to adjust the carrier wave\u2019s frequency and wet/dry mix."
                                    origin:origin
                                     width:250.0f];
    ...
    bbl = [[HelpBubble alloc] initWithText:@"Tap to activate high-pass filter..."
                                    origin:origin
                                     width:200.0f];
    [bbl setPointerLocation:kHelpBubblePointerLocationTop offset:CGPointMake(-65.0f, 0.0f)];
    [bbl setColor:effectsModeHelpColor];
    [[self view] addSubview:bbl];
...
//    /* If we're getting input from the Synth, we need to extend the time duration we're retrieving from the recording buffer to compensate for the phaze zero offset. Lower fundamental frequencies have larger offsets due to longer wavelengths. */
    ...
    startIdx = fmax(tdScopeView.visiblePlotMin.x, 0.0f) * audioController.sampleRate;
    endIdx = fmin(tdScopeView.visiblePlotMax.x * audioController.sampleRate, audioController.recordingBufferLengthFrames);
    visibleBufferLength = endIdx - startIdx;
    ...
    if (audioController.recordedPlayback) {     // Playback
        [audioController getPlaybackBuffer:tdScopeYDataDry from:startIdx to:endIdx];
        [audioController getPlaybackWetBuffer:tdScopeYDataWet from:startIdx to:endIdx];
    }
    
    else if (![audioController inputEnabled]) {  // Paused
...
- (void)toggleInput {
    ...
    /* TD scope is missing traces when input is toggled, so here's a hack to make it work */
    [self tdScopeBoundsChanged];
}
...
- (void)enableInput {
    ...
    [audioController setVisibleRangeInSeconds:fmaxf(0.0f, tdScopeView.visiblePlotMin.x)
    ...
    /* If delay taps are active, keep their positions relative to the visible plot minimum when toggling audio so they don't disappear beyond the visible bounds */
    for (int i = 0; i < [audioController getNumDelayTaps]; i++) {
        [tdControlArray setHorizontalValue:[audioController getDelayTimeForTap:i]
...
- (void)toggleDelay:(id)sender {
    ...
    /* Update list of active effects for MIDI mappings picker */
    AppDelegate *delegate = (AppDelegate *)[[UIApplication sharedApplication] delegate];
    [delegate queryAvailableMIDIParams];
}
...
//    NSLog(@"%s: fc = %f\tQ = %f", __PRETTY_FUNCTION__, [audioController->lpf cornerFrequency], [audioController->lpf Q]);
}
...
- (void)addDelayTap {
    ...
    if (!inputPaused)
        [(AppDelegate *)[[UIApplication sharedApplication] delegate] recordStateButtonPress:self];
...
//            if (delayUpdateCounter == 0) {
                ...
                if (inputPaused)
                    [audioController setDelayTap:idx time:vals.x-tdScopeView.visiblePlotMin.x
                                       amplitude:fmax(vals.y, 0.0)];
                else
                    [audioController setDelayTap:idx time:vals.x
                                   amplitude:fmax(vals.y, 0.0)];
            
//            }
//            else {
//                delayUpdateCounter++;
//                if (delayUpdateCounter == 3)
//                    delayUpdateCounter = 0;
//            }

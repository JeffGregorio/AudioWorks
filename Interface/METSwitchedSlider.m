...
METSwitchBehavior const kDefaultSwitchBehavior = METSwitchBehaviorSetSliderMin;
...
@synthesize switchStateChanged = _switchStateChanged;
@synthesize sliderValueChanged = _sliderValueChanged;
...
- (BOOL)beginTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
    ...
    else if (loc.x > _sliderLeftEdge && loc.x < _sliderRightEdge) {
...
- (BOOL)continueTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
    ...
    if (loc.x > _sliderLeftEdge && loc.x < _sliderRightEdge) {
        _sliderValueChanged = true;
        [self computeSliderFillRatioFromTouchLocation:loc];
        [self scaleSliderValue];
        [self computeTabLocation];
...
- (void)endTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
    ...
    /* If we began and ended the touch inside of the switch bounds */
    if (loc.x < _switchWidth && _switchState == METSwitchStateDown) {
        
        /* Toggle the switch state */
        _switchStateChanged = true;
        [self setSwitchState:_previousSwitchState == METSwitchStateOff ? METSwitchStateOn : METSwitchStateOff];
    }
    /* Otherwise, update the slider value */
    else if (loc.x > _sliderLeftEdge && loc.x < _sliderRightEdge) {
        _sliderValueChanged = true;
        [self computeSliderFillRatioFromTouchLocation:loc];
        [self scaleSliderValue];

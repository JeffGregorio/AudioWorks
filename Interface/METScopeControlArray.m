...
- (void)setStyle:(METControlStyle)style {
    ...
    switch (style) {
        ...
        case kMETControlStyleRect:
            ...
            borderWidth = 1.0;
            break;
        case kMETControlStylePlayhead:
            frame.size.width = dotSize.height;
            frame.size.height = dotSize.height;
            cornerRadius = dotSize.height / 6.0;
            borderWidth = 0.0;
            imgView = [[UIImageView alloc] initWithFrame:self.bounds];
            imgView.image = [UIImage imageNamed:@"Playhead.png"];
            [self addSubview:imgView];
            [self setFaceColor:[UIColor clearColor]];
            break;
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if ([self isEnabled]) {
        ...
        /* See if we're within the bounds of any controls */
        for (int i = 0; i < [controls count]; i++) {
            ...
            if (CGRectContainsPoint(testFrame, touch)) {
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    /* Reposition any active controls and notify the delegate */
...
#pragma mark - Interface Setters
- (void)setValues:(CGPoint)vals forControlWithTag:(NSInteger)tag {
    
    METControl *ctrl = [self getControlWithTag:tag];
    
    /* Must set control values in main thread for the UI to actually update */
    dispatch_async(dispatch_get_main_queue(),^ {
        [ctrl setValues:vals];
        if (delegate) [delegate parameterDotTouchUp:ctrl];
        [self setNeedsDisplay];
    });
}

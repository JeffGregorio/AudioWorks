...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (editable) {
        ...
        for (int i = 0; i < [harmonicDots count]; i++) {
            ...
            dotFrame = [[harmonicDots objectAtIndex:i] frame];
            
            if (CGRectContainsPoint(dotFrame, previousTouchLocation)) {
                activeDotIndex = i;
...
//    if (touchLoc.y < 0.0f || touchLoc.y > parentScope.frame.size.height) {
...
//    }
    ...
    if (activeDotIndex < [harmonicDots count]) {
        ...
        if (unscaledAmplitude <= 1.0f) {
            [activeDot setAmplitude:20.0f * log10f(unscaledAmplitude + 0.0001f)];
    ...
    /* Reposition the noise handle if active and notify the delegate */
        ...
        if (unscaledAmplitude <= 1.0f) {
            [noiseHandle setAmplitude:20.0f * log10f(unscaledAmplitude + 0.0001f)];

...
- (void)setDefaults {
    ...
    pinchZoomMode = kMETScopeViewPinchZoomHoldMin;
...
- (void)setPlotUnitsPerTick:(float)xTick vertical:(float)yTick {
    ...
    if (visibleRange.x == 0 || visibleRange.y == 0)
        return;
...
//    CGPoint modulus;
//    modulus.x = 5.0f * powf(10.0f, orderOfMag.x);
//    modulus.y = 5.0f * powf(10.0f, orderOfMag.y);
//    
//    NSLog(@"orderOfMag = [%f, %f]", orderOfMag.x, orderOfMag.y);
//    NSLog(@"   modulus = [%f, %f]", modulus.x, modulus.y);
    ...
    if (xGridAutoScale) {
        ...
        while (ticksInFrame.x > METScopeView_AutoGrid_MaxXTicksInFrame) {
            tickUnits.x *= 2.0;
            ticksInFrame.x = visibleRange.x / tickUnits.x;
        }
        
        while (ticksInFrame.x < METScopeView_AutoGrid_MinXTicksInFrame) {
            tickUnits.x /= 2.0;
            ticksInFrame.x = visibleRange.x / tickUnits.x;
        }
    ...
    if (yGridAutoScale) {
        ...
        /* Double or halve the tick units if we've got too few or too many within visible bounds */
        while (ticksInFrame.y > METScopeView_AutoGrid_MaxYTicksInFrame) {
            tickUnits.y *= 2.0;
            ticksInFrame.y = visibleRange.y / tickUnits.y;
        }
        
        while (ticksInFrame.y < METScopeView_AutoGrid_MinYTicksInFrame) {
            tickUnits.y /= 2.0;
            ticksInFrame.y = visibleRange.y / tickUnits.y;
        }
...
- (bool)getVisibilityAtIndex:(int)idx {
    
    if (idx >= 0 && idx < plotDataSubviews.count)
        return [((METScopePlotDataView *)plotDataSubviews[idx]) visible];
    else {
        NSLog(@"Invalid plot data index %d\nplotDataSubviews.count = %lu", idx, (unsigned long)plotDataSubviews.count);
        return false;
    }
